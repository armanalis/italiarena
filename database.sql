...
-- Builds a 10-question playlist for one match:
-- 3 grammar, 3 vocabulary, 3 fill-in-the-blank, 1 idioms.
-- Skips anything the player has already seen and picks randomly via random_float.
...
begin
  ...
  -- First match for a new player — make sure they have a stats row.
  ...
  with selected as (
    ...
    (
      ...
      where language = p_language
        and level = p_level
        and category = 'fill-in-the-blank'
        and not (id = any (v_seen))
        and random_float > random()
      order by random_float
      limit 3
